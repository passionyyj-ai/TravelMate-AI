...
function setTranslatorVoiceStatus(message, active = false) {
  ...
  if (button) {
    ...
    button.textContent = active ? "⏹ 말하기 끝" : "🎤 말하기 시작";
...
async function transcribeTranslatorRecording(blob) {
  ...
  setTranslatorVoiceStatus("말씀을 알아들었습니다. 번역하고 있습니다…", true);
  await translate();
  setTranslatorVoiceStatus("번역이 완료되었습니다. 필요하면 ‘발음 듣기’를 누르세요.");
}
...
async function startTranslatorRecorder() {
  ...
  recorder.onstop = async () => {
    ...
    try {
      ...
      setTranslatorVoiceStatus("목소리를 알아듣지 못했습니다. 휴대폰을 가까이 두고 다시 말씀해 주세요.");
    }
  };
  recorder.start();
  setTranslatorVoiceStatus("지금 말씀하세요. 말씀을 마치면 ‘말하기 끝’을 누르세요.", true);
...
function stopTranslatorRecorder() {
  ...
  state.translatorRecorderStream?.getTracks().forEach(track => track.stop());
  state.translatorRecorderStream = null;
  setTranslatorVoiceStatus("‘말하기 시작’을 누른 뒤 천천히 말씀하세요.");
}
