...
async function stopTranslatorRecorder() {
  ...
  if (state.translatorPcmRecorder?.state === "recording") {
    ...
    try {
      ...
      await transcribeTranslatorRecording(blob);
    } catch (error) {
      setTranslatorVoiceStatus("음성 인식 실패: " + error.message);
    }
    return;
...
async function speechRecognize() {
  if (state.translatorPcmRecorder?.state === "recording" || state.translatorRecorder?.state === "recording") {
    await stopTranslatorRecorder();
    return;
  }
  try {
...
function downsamplePcm(input, inputRate, outputRate = 16000) {
...
async function startPcmWavRecorder() {
  if (!navigator.mediaDevices?.getUserMedia) throw new Error("이 기기에서는 마이크를 사용할 수 없습니다.");
  const stream = await navigator.mediaDevices.getUserMedia({
  ...
  if (!AudioContextClass) {
    ...
    throw new Error("이 브라우저는 WAV 녹음을 지원하지 않습니다.");
  }
  ...
  return {
    ...
    async stop() {
      ...
      stream.getTracks().forEach(track => track.stop());
      const sourceRate = context.sampleRate || 48000;
      try { await context.close(); } catch {}
      const merged = mergeFloat32Chunks(chunks);
      if (merged.length < sourceRate * 0.35) throw new Error("말소리가 너무 짧습니다. 1초 이상 말씀해 주세요.");
      const pcm16k = downsamplePcm(merged, sourceRate, 16000);
      return pcmToWavBlob(pcm16k, 16000);
    },
    cancel() {
      stopped = true;
      this.state = "inactive";
      processor.onaudioprocess = null;
      try { source.disconnect(); processor.disconnect(); silentGain.disconnect(); } catch {}
      stream.getTracks().forEach(track => track.stop());
      context.close().catch(() => {});
    },
  };
...
async function stopAiVoiceRecognition() {
  if (state.aiRecognition) {
    try { state.aiRecognition.stop(); } catch {}
    state.aiRecognition = null;
  }
  ...
  if (state.aiPcmRecorder?.state === "recording") {
    ...
    try {
      const blob = await recorder.stop();
      await transcribeAiRecording(blob);
    } catch (error) {
      setAiVoiceStatus("음성 인식 실패: " + error.message);
    }
    return;
...
const TRAVELMATE_APP_VERSION = "3.0.4.1";
